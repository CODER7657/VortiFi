...
<body>
  ...
  <ul>
    ...
    <li><a href="#frontend-overview">Frontend Overview</a>
      <ul>
        <li><a href="#configjs-contract-connection">config.js — Contract Connection</a></li>
        <li><a href="#appjs-routing-authentication">App.js — Routing & Authentication</a></li>
        <li><a href="#admin-dashboard">Admin Dashboard</a></li>
        <li><a href="#voting-page">Voting Page</a></li>
      </ul>
    </li>
  ...
  <p>VortiFi offers a secure, auditable blockchain voting platform. Voters receive unique tokens from the admin, ensuring each token is single-use and vote integrity is preserved. The React frontend connects via ethers.js to interact with the Ethereum smart contract.</p>

  <h2 id="smart-contract-overview">Smart Contract Overview</h2>
  <h3>Contract: <code>RotaractVoting.sol</code></h3>
  <ul>
    <li>Admin controls candidate addition/removal and voter token issuance.</li>
    <li>Candidates have name, position, vote count, and active status (soft deletion).</li>
    <li>Tokens are securely managed to allow only valid, unused votes.</li>
    <li>Votes are stored with voter IDs and candidate information.</li>
    <li>Public can view active candidates and results, filterable by position.</li>
  </ul>
  <h4>Key Functions:</h4>
  <table>
    <tr>
    ...
    <tr>
      <td><code>addCandidate(name, position)</code></td>
      <td>Admin only</td>
      <td>Add new candidate</td>
    </tr>
    <tr>
      <td><code>removeCandidate(id)</code></td>
      <td>Admin only</td>
      <td>Deactivate a candidate</td>
    </tr>
    <tr>
      <td><code>addVoterToken(token, voterId)</code></td>
      <td>Admin only</td>
      <td>Issue unique voting token</td>
    </tr>
    <tr>
      <td><code>vote(token, candidateId)</code></td>
      <td>Public</td>
      <td>Cast vote with token (hashed & verified)</td>
    </tr>
    <tr>
      <td><code>getAllVotes()</code></td>
      <td>Admin only</td>
      <td>View all voting records</td>
    </tr>
    <tr>
      <td><code>getResults()</code></td>
      <td>Public</td>
      <td>View active candidates and vote counts</td>
    </tr>
    <tr>
      <td><code>getResultsByPosition(pos)</code></td>
      <td>Public</td>
      <td>Filter results by position</td>
    </tr>
  </table>
  ...
  <ul>
    ...
    <li>Frontend components use it to interact with the smart contract.</li>
    <li>Replace <code>"here your deployed contract ADDRESS"</code> with the actual contract address after deployment.</li>
  </ul>

  <h3 id="appjs-routing-authentication"><code>App.js</code> — Routing & Authentication</h3>
  <ul>
    <li>Handles routing between Voting page, Admin login, and Admin dashboard.</li>
    <li>Maintains admin login state with localStorage and 15-minute session expiry.</li>
    <li>Protects admin routes from unauthorized access.</li>
    <li>Displays navigation links based on login status.</li>
  </ul>

  <h3 id="admin-dashboard">Admin Dashboard (<code>AdminDashboard.js</code>)</h3>
  <ul>
    <li>Restricted to the admin wallet address stored on-chain.</li>
    <li>Add/remove candidates; issue unique hashed voter tokens; monitor live voting results and voter activity.</li>
    <li>Interacts directly with contract via ethers.js, requires MetaMask or similar.</li>
  </ul>

  <h3 id="voting-page">Voting Page (<code>VotingPage.js</code>)</h3>
  <ul>
    <li>Voters enter their unique token and select a candidate.</li>
    <li>Token is hashed client-side before submission to ensure security.</li>
    <li>Immediate feedback on vote success/failure.</li>
    <li><b>Security note:</b> Replace hardcoded private key with user wallet signing for production.</li>
  </ul>
...
cd VortiFi</code></pre>
    ...
    <li><b>Deploy the contract</b> (using Hardhat or Remix), and update <code>config.js</code> with the contract address.</li>
